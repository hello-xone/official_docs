---
description: Fast Sync Using Snapshots
keywords: 'Xone, blockchain, using-snapshots'
---
...
<Tabs items={['Mainnet', 'Testnet']} defaultValue="Mainnet">
  <Tabs.Tab>
    ...
    SNAP_DATE=$(curl -s https://xone-mainnet-files.s3.ap-southeast-1.amazonaws.com/latest-version-snapshot.txt)
  ...
  <Tabs.Tab>
    ```bash showLineNumbers copy
    # Snapshot date: recommended to use yesterday’s date, format YYYYMMDD
    SNAP_DATE=$(curl -s https://xone-testnet-files.s3.ap-southeast-1.amazonaws.com/latest-version-snapshot.txt)
