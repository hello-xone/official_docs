...
<Tabs items={['Mainnet', 'Testnet']} defaultValue="Mainnet">
  <Tabs.Tab>
    ...
    SNAP_DATE=$(curl -s https://xone-mainnet.s3.ap-southeast-1.amazonaws.com/latest-version-snapshot.txt)

    # Snapshot filename and download URL
    SNAP_FILE="snapshot-${SNAP_DATE}.tar.lz4"
    SNAP_URL="https://xone-mainnet.s3.ap-southeast-1.amazonaws.com/snapshots/${SNAP_FILE}"
  ...
  <Tabs.Tab>
    ```bash showLineNumbers copy
    # Snapshot date: recommended to use yesterday’s date, format YYYYMMDD
    SNAP_DATE=$(curl -s https://xone-testnet-file.s3.ap-southeast-1.amazonaws.com/latest-version-snapshot.txt)

    # Snapshot filename and download URL
    SNAP_FILE="snapshot-${SNAP_DATE}.tar.lz4"
    SNAP_URL="https://xone-testnet-file.s3.ap-southeast-1.amazonaws.com/snapshots/${SNAP_FILE}"
