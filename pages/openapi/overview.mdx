...
keywords: 'Xone, blockchain, overview'
---
...
<Callout emoji="⚡">
  [Xone](https://xone.org/) Chain’s public RPC endpoints are your gateway to the network. Every on‑chain action—querying balances, sending transactions, or deploying smart contracts—starts with a JSON‑RPC call.
</Callout>

<Link className='text-primaryHue' isExternal showAnchorIcon href="/study/xone">[Xone](https://xone.org/) Chain is an L1 public chain based on Ethereum</Link>, fully compatible with <Link className='text-primaryHue' isExternal showAnchorIcon href="https://ethereum.org/en/developers/docs/apis/json-rpc/">Ethereum JSON-RPC API</Link>, and supports interaction with mainstream libraries such as <Link className='text-primaryHue' isExternal showAnchorIcon href="https://web3js.readthedocs.io/en/v1.10.0/">Web3.js</Link> and <Link className='text-primaryHue' isExternal showAnchorIcon href="https://docs.ethers.org/v6/">Ethers.js</Link>. Developers can use JSON-RPC to access [Xone](https://xone.org/) Chain for account management, transaction sending, smart contract calling, and other operations.

## Endpoint

[Xone](https://xone.org/) Chain provides multiple RPC endpoints for developers to connect to the main network or test network. All requests use the POST method.
...
<div className="overflow-x-auto">
  <Table key={new Date().getTime()} removeWrapper aria-label="json prc endpoint table" className="mt-2">
    ...
    <TableBody>
      <TableRow key="1">
        <TableCell>[Xone](https://xone.org/) Mainnet</TableCell>
        <TableCell>
        `https://rpc.xone.org`\
        `wss://rpc.xone.org`
        </TableCell>
      ...
      <TableRow key="2">
        <TableCell>[Xone](https://xone.org/) Testnet</TableCell>
        <TableCell>
